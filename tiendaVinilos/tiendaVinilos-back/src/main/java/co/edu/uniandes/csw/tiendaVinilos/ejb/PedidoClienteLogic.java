...
public class PedidoClienteLogic 
{
     ...
     */
    public PedidoClienteEntity updateEstadoPedido(Long id, PedidoClienteEntity entity) throws BusinessLogicException {
     ...
     * @return el pedido con los cambios actualizados en la base de datos.
     */
    public PedidoClienteEntity updateInfoPedido(Long id, PedidoClienteEntity entity) throws BusinessLogicException {
        LOGGER.log(Level.INFO, "Inicia proceso de actualizar pedido con id={0}", id);
        PedidoClienteEntity pedido = persistence.find(id);
        if (pedido == null) {
            throw new BusinessLogicException( "El Pedido con el id {0} no existe" + id);
     ...
     *
     * @param id: id del pedido para buscarlo en la base de datos.
     * @param entity: pedido con los cambios para ser cancelado, puede ser la direccion o el telefono de contacto.
     * @return el pedido con los cambios actualizados en la base de datos.
     */
    public PedidoClienteEntity cancelPedido(Long id, PedidoClienteEntity entity) throws BusinessLogicException {
        LOGGER.log(Level.INFO, "Inicia proceso de cancelar pedido con id={0}", id);
        PedidoClienteEntity pedido = persistence.find(id);
        if (pedido == null) {
            throw new BusinessLogicException( "El Pedido con el id {0} no existe" + id);
        }
        ...
        // Note que, por medio de la inyección de dependencias se llama al método "update(entity)" que se encuentra en la persistencia.
        PedidoClienteEntity newEntity = persistence.update(entity);
        LOGGER.log(Level.INFO, "Termina proceso de cancelar pedido con id={0}", entity.getId());
        return newEntity;
    }
